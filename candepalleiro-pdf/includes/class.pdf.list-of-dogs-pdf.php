...
//inicimaos a clase
class ListarPerrosFpdf extends FPDF {
	...
	function print_info(){
		...
		for($i=0;$i<count($url_array);$i++):
			...
			switch($parameter[0]):
				...
				break;
				case 'find_candepalleiro_confirmado':
					$confirmado=true;
				break;
				case 'find_candepalleiro_no_confirmado':
					$no_confirmado=true;
				break;
				case 'find_candepalleiro_con_defectos':
					$con_defectos=true;
				break;
				case 'find_candepalleiro_sin_defectos':
					$sin_defectos=true;
				break;
				case 'find_candepalleiro_baja':
					$baja=true;
				break;
				case 'find_candepalleiro_no_baja':
					$no_baja=true;
				break;
				case 'de':
					$de=urldecode($parameter[1]);
				break;
				case 'a':
					$a=urldecode($parameter[1]);
				break;
				case 'orderby':
					$orderby=$parameter[1];
				break;
		...
		if(!empty($de) and !empty($a)):
			$where .= ' AND nacemento BETWEEN \''.$this->invert_data($de).'\' AND \''.$this->invert_data($a).'\' ';
		endif;
		...
		$querydata = $wpdb->get_results(
			'
			SELECT * FROM `'.$wpdb->prefix.'candepalleiro_can`
		...
		$this->AddPage('L');
		$config= array(utf8_decode(__('Nombre','can-de-palleiro-fpdf')),utf8_decode(__('RLX','can-de-palleiro-fpdf')),utf8_decode(__('Capa','can-de-palleiro-fpdf')),utf8_decode(__('Sexo','can-de-palleiro-fpdf')),utf8_decode(__('Ayuntamiento','can-de-palleiro-fpdf')),utf8_decode(__('Nacemiento','can-de-palleiro-fpdf')),utf8_decode(__('Propietario','can-de-palleiro-fpdf')));
		$this->style="B";
		$this->size="12";
		$this->drawRow($config,array(152,97,97,40,97,97,194),14);
		...
		foreach($querydata as $can){
			$config= array(utf8_decode($can->nome),utf8_decode($can->rlx),utf8_decode($can->capa),utf8_decode($can->sexo),utf8_decode($can->concello),$this->invert_data(utf8_decode($can->nacemento)),utf8_decode($can->p_nome).' '.utf8_decode($can->p_apelido));
			$this->drawRow($config,array(152,97,97,40,97,97,194),14);
		}
		endif;
		//Closure line
