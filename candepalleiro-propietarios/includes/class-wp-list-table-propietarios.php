...
class Wf_Propietarios_List extends WP_List_Table {
	...
	function column_p_apelido( $item ) {
		...
		$actions = [
					'edit'      => sprintf('<a href="?page=%s&action=%s&codex=%s&amp;_wpnonce=%s"><i title="'.__('Información del propietario','candepalleiro-management').'" class="fa fa-info-circle"><span class="hide">&nbsp;'.__('Información del propietario','candepalleiro-management').'</span></i></a>',$_REQUEST['page'],'edit',$item['id_propietario'],wp_create_nonce( 'wf_edit_propietario'.$item['id_propietario'])),			
					];
		else:
		$actions = [
					'edit'      => sprintf('<a href="?page=%s&action=%s&codex=%s&amp;_wpnonce=%s"><i title="'.__('Modificar','candepalleiro-management').'" class="fa fa-pencil-square-o"><span class="hide">&nbsp;'.__('Modificar','candepalleiro-management').'</span></i></a>',$_REQUEST['page'],'edit',$item['id_propietario'],wp_create_nonce( 'wf_edit_propietario'.$item['id_propietario'])),			
					];
		endif;
	...
	public function get_bulk_actions() {
		if(!current_user_can('edit_canis_canis')):
		$actions = [
			'list-of-owners-pdf'    => __('Listado de Propietarios (PDF)','candepalleiro-management'),	
		];
		else:
		$actions = [
			'list-of-owners-pdf'    => __('Listado de Propietarios (PDF)','candepalleiro-management'),	
			...
			'------'=>'------',
			'------'=>'------',
			'trash-owner'    => __('Papelera','candepalleiro-management'),
		];
		endif;
	...
	public function process_bulk_action() {
		...
		if ( 'trash-owner' === $this->current_action() and current_user_can('edit_canis_canis')){
			$ids=array();
			...
			if(empty($ids)):
				...
				$wpdb->query(
					$wpdb->prepare(
						"UPDATE {$wpdb->prefix}candepalleiro_propietario SET trash=1 
						 WHERE id_propietario IN (%s)",$ids
					)
				);
				wp_redirect("?page=propietarios");
				exit;
			endif;
			//print_r($_REQUEST['bulk-customer']);
		}

		//If the bulk-spam-no bulk action is triggered
		if ( 'list-of-owners-pdf' === $this->current_action()){
				$url=str_replace('page=propietarios','page=PDF',esc_url_raw(add_query_arg()));
			    wp_redirect($url);
			    exit;
		}
	...
	public function prepare_items() {
		...
		$per_page     = $this->get_items_per_page( 'propietarios_per_page', 100 );
	...
	public  function get_propietarios( $per_page = 5, $page_number = 1 ) {
		...
		$sql = "SELECT * FROM {$wpdb->prefix}candepalleiro_propietario WHERE trash<1";
	...
	public  function record_count_search() {
		...
		$sql = "SELECT COUNT(*) FROM {$wpdb->prefix}candepalleiro_propietario WHERE trash<1";
		$sql .= " ".self::data_search()." "; //añadido al código original
		return $wpdb->get_var( $sql );
...
class Wf_Propietarios_List_load{
	...
	?>
		<div class="wrap">	    
			<?php	
		     printf('<h2 class="fa fa-table"> %s </h2><a href="?page=nuevo-propietario" class="add-new-h2 fa fa-plus">&nbsp;%s</a>',esc_html__('Listado de propietarios','candepalleiro-management'),esc_html__('Nuevo propietario','candepalleiro-management'));
		    ?>
		   <div id="display-search-form">
		    ...
		    <ul class="subsubsub">
		        <li class="all"><a class="current" href="admin.php?page=propietarios"><?php _e("Todos",'candepalleiro-management');?> <span class="count">(<?php echo $Wftable->record_count();?>)</span></a> |</li>
		        <li class="trash"><a href="admin.php?page=propietarios&post_type=papelera"><?php _e("Papelera",'candepalleiro-management');?> <span class="count">(<?php echo $Wftable->record_count(1);?>)</span></a></li>
		    </ul> 
